بصفتي **مدير تخطيط المشاريع (Project Planning Manager)**، وبناءً على المنهجية المعتمدة في **المصدر** لتصنيف المشاريع، وتوفر البيانات الخام في الأبحاث (المصادر 1-6)، إليك الوثيقة المرجعية الأولى.

هذا الملف هو "الدستور" الذي سنعود إليه في كل خطوة قادمة.
...
## 1. 🆔 Project Identity Card (بطاقة المشروع)
...
| **Theme Base** | **AstroWind** (Selected for RTL support & Business readiness). |
| **Key Features** | Fuzzy Search, Trust Boxes, Before/After Cases, Lazy-loaded Video Embeds. |
| **Target Audience** | Jeddah residents/businesses in "Panic Mode" (Data Loss). |

---

## 2. 🗺️ Documentation Map (خريطة الوثائق)

According to **"Projects Methodology v1.0"** (Source 7) for **Medium Projects**, the following artifacts are **MANDATORY** before writing any code:

| File Name | Description | Status | Source Data Availability |
| :--- | :--- | :--- | :--- |
...
## 3. 🚀 Execution Sequence (تسلسل التنفيذ)

We will generate the documents in this logical order to build a solid foundation:

1.  **Vision & Risks (`01` & `02`):** First, we lock the strategy and identify the "Red Lines" (SEO risks).
2.  **Requirements (`03_PRD`):** Define *what* we are building (features like Fuzzy Search & Case Studies).
3.  **Visuals (`04_UX`):** Define *how* it looks (adapting AstroWind to the Saudi local trust vibe).
4.  **Architecture (`05_TAD`):** Define the code structure (Collections, Components, API routes) to support the requirements.
5.  **Governance (`06_Rules`):** Set the boundaries for the AI Coder to ensure clean, high-performance code.

---

## 4. 🔗 Critical Decisions Log (سجل القرارات الحاسمة)

*   **Theme Decision:** Selected **AstroWind** over AstroPaper (Better for "Business/Services" look + RTL).
